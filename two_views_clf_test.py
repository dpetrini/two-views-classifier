...
def main():
    ...
    ap.add_argument("--mode", required=True, help="mode of testing: test_two or test_many")
    ...
    if args["mode"] == "test_two":
        ...
        assert args["data"] is not None, "You should specify test data csv for mode `test_many`"
        path_annotation =  args["data"]
        df_ann = pd.read_csv(path_annotation).sample(frac=1, random_state=42).reset_index(drop=True)
        df_ann["Predict_prob"] = [float("nan")] * df_ann.shape[0]
        path_save = path_annotation.replace(".csv", "_preds.csv")
        for i, patient in enumerate(tqdm(df_ann["Patient_id"].unique(), desc = "Patient")):
            df_patient_L = df_ann[(df_ann["Patient_id"] == patient) & (df_ann["Laterality"] == "L")]
            df_patient_R = df_ann[(df_ann["Patient_id"] == patient) & (df_ann["Laterality"] == "R")]
            pred_L = get_patient_predict(df_patient_L, device, model, use_aug)
            pred_R = get_patient_predict(df_patient_R, device, model, use_aug)
            df_ann.loc[(df_ann["Patient_id"] == patient) & (df_ann["Laterality"] == "L"), "Predict_prob"] = pred_L
            df_ann.loc[(df_ann["Patient_id"] == patient) & (df_ann["Laterality"] == "R"), "Predict_prob"] = pred_R
            if (i + 1) % 20 == 0:
                df_ann.to_csv(path_save, index = False)
                logger.info(f"Dataframe with predictions saved to {path_save} on patient {i}")
                try:
